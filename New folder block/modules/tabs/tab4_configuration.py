...
def render_tab4():
    ...
    # ── E. State-Space Covariance Blocks (Q / P0) ─────────────────────
    st.markdown("### E · State-Space Covariance Blocks (Q / P₀)")
    per_channel_info(
        "🧱 The process-noise (Q) and initial-covariance (P₀) matrices are "
        "built as <b>block-diagonal</b> — an <b>intercept block</b> and a "
        "<b>betas block</b> (every media/comp-media/non-media/comp-non-media/"
        "price coefficient state, together), with <b>zero</b> covariance "
        "between the two blocks. Within the betas block you can optionally "
        "let all betas share a common (equicorrelated) noise/uncertainty "
        "component instead of evolving fully independently — leave ρ at 0 "
        "for the standard fully-independent behaviour."
    )
    qc1, qc2 = st.columns(2)
    with qc1:
        q_intercept_var = st.number_input(
            "Intercept block noise (Q_intercept)", 1e-8, 1.0, 1e-4,
            format="%.2e", key="q_intercept_var",
            help="Process-noise variance of the intercept's own block.")
        beta_process_corr = st.slider(
            "Betas block noise correlation ρ (Q)", 0.0, 0.9, 0.0, 0.05,
            key="beta_process_corr",
            help="0 = fully independent process noise per beta (default). "
                 ">0 = betas share a common process-noise component.")
    with qc2:
        q_beta_var = st.number_input(
            "Betas block noise (Q_beta)", 1e-9, 1.0, 1e-6,
            format="%.2e", key="q_beta_var",
            help="Process-noise variance shared by every beta in the block.")
        beta_prior_corr = st.slider(
            "Betas block prior correlation ρ₀ (P₀)", 0.0, 0.9, 0.0, 0.05,
            key="beta_prior_corr",
            help="0 = fully independent initial uncertainty per beta "
                 "(default). >0 = betas start with a shared/correlated "
                 "prior uncertainty component.")
    ...
    if st.button("💾 Save Configuration", type="primary", use_container_width=True):
        if not media:
            ...
            st.session_state.config = {
                ...
                "q_intercept_var": float(q_intercept_var),
                "q_beta_var": float(q_beta_var),
                "beta_process_corr": float(beta_process_corr),
                "beta_prior_corr": float(beta_prior_corr),
